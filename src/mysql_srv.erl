...
handle_cast({fetch, PoolId, Pid, Ref, Query}, State) ->
	case pool_get(PoolId) of
		#mysql_pool{} = Pool ->
			...
			pool_put(PoolId, Pool2);
		_ ->
			Pid ! {error, Ref, lists:concat(["PoolId ", PoolId, " not exist!"])}
	end,
	{noreply, State};
...
heart_handle() ->
	...
	Fun = fun
			  (PoolId) ->
				  case pool_get(PoolId) of
					  #mysql_pool{pool_ready=Readys} ->
						  [mysql_side_srv:heart_cast(SideSrvName) || SideSrvName <- Readys];
					  _ ->
						  ?skip
				  end
		  end,
...
check_handle() ->
	Time	= mysql_util:seconds(),
	Pools	= pools_get(),
	check_handle(Time, Pools).

check_handle(_Time, []) ->
	ok;
check_handle(Time, [PoolId|PoolIds]) ->
	case pool_get(PoolId) of
		#mysql_pool{pool_working=Workings} = Pool ->
			Workings2 = check_handle(Time, Workings, []),
			pool_put(PoolId, Pool#mysql_pool{pool_working=Workings2}),
			check_handle(Time, PoolIds);
		_ ->
			check_handle(Time, PoolIds)
	end.

check_handle(_Time, [], Workings) ->
	Workings;
check_handle(Time, [{_Ref,_FromPid,TimeQuery,SideSrvName} = Working|Workings], WorkingsAcc) ->
	case Time - TimeQuery >= ?MYSQL_TIMEOUT_RUN of % SideSrv重起
		true ->
			?ERROR("MySQL side server timeout!"),
			mysql_side_sup:restart_child(SideSrvName),
			check_handle(Time, Workings, WorkingsAcc);
		false ->
			check_handle(Time, Workings, [Working|WorkingsAcc])
	end.



heart_register() ->
	erlang:send_after(timer:seconds(?MYSQL_HEART_TIME), erlang:self(), heart).
